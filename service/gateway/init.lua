...
s.resp.send = function(source, playerid, msg)--发送消息到客户端，这个函数由其他的sevice调用
...
s.resp.quit = function(source,playerid)
    ...
    local conn = conns[c.fd]
    if not conn.playerid then --还没完成登录
        return 
    else
        players[conn.playerid] = nil
        skynet.call("agentmgr", "lua", "reqquit", playerid, "quit登录")
        conn.playerid = nil
    end
end
