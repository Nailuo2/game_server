...
s.resp.reqkick = function(source, playerid)--请求下线，向agent和gate发送下线命令
	local mplayer = players[playerid]
	if not mplayer then
		return false
	end
...
s.resp.reqquit =function(source,playerid)
	local mplayer = players[playerid]
	if not mplayer then
		return false
	end
	...
	local pgate = mplayer.gate
	mplayer.status = STATUS.LOGOUT
	s.send(pnode, pgate, "send", playerid, {playerid.." 退出登录"})--向客户端发送下线信息
	s.call(pnode, pgate, "quit", playerid)
	players[playerid] = nil
	skynet.error("退出登录")
	return true
end
